...
gene_filtering = function(count_df, normalized_count_df){
	...
	passed_genes <- apply(count_df, 1, function(x) (sum(x >= as.numeric(config$parameter$count_threshold))/length(x)) >= as.numeric(config$parameter$sample_frac_threshold))
	if (exists("normalized_count_df", mode="list")) {
		passed_norm_thres <- apply(normalized_count_df, 1, function(x) (sum(x >= as.numeric(config$parameter$normalized_count_threshold))/length(x)) >= as.numeric(config$parameter$sample_frac_threshold))
		passed_genes <- (passed_genes & passed_norm_thres)
	}
	included_gene_list <- rownames(count_df)[passed_genes]
	cat(paste0("=> passed genes: ", length(included_gene_list),"\n"), file=log_file)

	### Apply mappability threshold
	if (!is.na(config$file$mappability_scores)) {		
		mappability_scores <- read.table(config$file$mappability_scores, header=F, col.names=c("gene_id","score"))
		high_mappability_genes <- subset(mappability_scores, score >= config$parameter$mappability_threshold)$gene_id
		included_gene_list <- intersect(high_mappability_genes, included_gene_list)
		cat(paste0("=> high mappability genes: ", length(included_gene_list),"\n"), file=log_file)
	}
...
prepare_dataframes <- function(mode="dea"){
	...
	count_df = read_quantification_matrix(config$file$read_count_matrix)
	
	## open normalized count file if provided
	if (!is.na(config$file$normalized_count_matrix)) {
		normalized_count_df = read_quantification_matrix(config$file$normalized_count_matrix)
	} else {
	...
	included_sample_df <<- read.csv(config$file$included_samples, sep="\t", header=T)
...
surrogate_variable_analysis <- function(normalized_count_df, design_df){
	...
	null_model_matrix = model.matrix(eval(parse(text=paste0("~",config$parameter$null_model))), data=design_df)
...
latent_variable_evaluation <- function(voom, design, type="SV"){
	...
	if (!is.na(config$file$gene_annotations)){
		annot <- read.csv(config$file$gene_annotations, sep="\t")
...
run_limma <- function(count_df, design_df){
	...
	if (contrast != config$parameter$contrast) {stop(paste0("limma is using ",contrast," as contrast"))} 
	...
	if (!is.na(config$file$gene_annotations)) {
		gene_description = read.csv(config$file$gene_annotations, sep="\t")
...
run_deseq <- function(count_df, design_df){
	...
	i = grep(config$parameter$contrast, full_model_list)
	no_contrast_model_list = full_model_list[-i]
	full_model_formula = paste0("~",paste0(no_contrast_model_list, collapse="+"),"+",config$parameter$contrast)
	...
	results <- data.frame(DESeq2::results(dds, name=config$parameter$contrast))
	results <- cbind(ID=rownames(results), results)	
